...
#define total_retry 10
static int heartbeat = (5 * 60); 
...
{
	u32 val;
	u32 debug_ptv, debug_src;

    
    #if 0
	val = wdt->timeout * 1000000ul / 2;
	#else
	val = wdt->timeout * 1000000ul;
    #endif	
    printk("dog, +tegra_wdt_enable, val=%d=0x%x\n", val, val);	
...
static int tegra_wdt_resume(struct platform_device *pdev)
{
	...
	if (wdt->enabled) {
    	wdt->enabled = true;
    	tegra_wdt_set_timeout(wdt, heartbeat/2);  
    	tegra_wdt_enable(wdt); 	
	}
