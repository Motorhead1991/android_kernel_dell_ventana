...
#define mxl_dbg(fmt, arg...) \
	if (mxl111sf_tuner_debug) mxl_printk(KERN_DEBUG, fmt, ##arg)
...
static int mxl1x1sf_tuner_top_master_ctrl(struct mxl111sf_tuner_state *state, int onoff)
{
	return (state->cfg->top_master_ctrl) ?
		state->cfg->top_master_ctrl(state->mxl_state, onoff) :
#if 0
		mxl1x1sf_top_master_ctrl(state->mxl_state, onoff);
#else
		-EINVAL;
#endif
}
...
static struct mxl111sf_reg_ctrl_info *mxl111sf_calc_phy_tune_regs(u32 freq, u8 bw)
{
	u8 filt_bw;
...
static int mxl1x1sf_tuner_get_lock_status(struct mxl111sf_tuner_state *state,
					  int *rf_synth_lock, int *ref_synth_lock)
{
	int ret;
	u8 data;
