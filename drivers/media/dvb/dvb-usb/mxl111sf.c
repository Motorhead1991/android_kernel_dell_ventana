...
MODULE_PARM_DESC(debug, "set debugging level (1=info, 2=xfer, 4=i2c, 8=reg, 16=adv (or-able))." DVB_USB_DEBUG_STATUS);

int dvb_usb_mxl111sf_isoc;
module_param_named(isoc, dvb_usb_mxl111sf_isoc, int, 0644);
MODULE_PARM_DESC(isoc, "enable usb isoc xfer (0=bulk, 1=isoc)." DVB_USB_DEBUG_STATUS);
...
int mxl111sf_read_reg(struct mxl111sf_state *state, u8 addr, u8 *data)
{
	...
	if (mxl_fail(ret)) {
		err("error reading reg: 0x%02x", addr);
		goto fail;
	}
...
({									\
	...
	if (mxl_fail(___ret)) {						\
		err("failed to get chip info on first probe attempt");	\
		___ret = mxl1x1sf_get_chip_info(state);			\
		if (mxl_fail(___ret))					\
			err("failed to get chip info during probe");	\
		else							\
			warn("WARNING: probe needed a retry "		\
			     "in order to succeed.");			\
	}								\
	___ret;								\
})
...
static void mxl111sf_get_model_no(struct dvb_frontend *fe)
{
	...
	c->vendor_model_no = state->tv.model;
}
...
static int mxl111sf_lgdt3305_frontend_attach(struct dvb_usb_adapter *adap)
{
	...
	if ((adap->fe = dvb_attach(lgdt3305_attach,
				   &hauppauge_lgdt3305_config,
				   &adap->dev->i2c_adap)) != NULL) {
		adap_state->fe_init = adap->fe->ops.init;
		adap->fe->ops.init = mxl111sf_adap_fe_init;
		adap_state->fe_sleep = adap->fe->ops.sleep;
		adap->fe->ops.sleep = mxl111sf_adap_fe_sleep;
		return 0;
	}
	ret = -EIO;
fail:
...
static int mxl111sf_lg2160_frontend_attach(struct dvb_usb_adapter *adap)
{
	...
	if ((adap->fe = dvb_attach(lg2160_attach,
...
static int mxl111sf_attach_demod(struct dvb_usb_adapter *adap)
{
	...
	if ((adap->fe = dvb_attach(mxl111sf_demod_attach, state,
				   &mxl_demod_config)) != NULL) {
		adap_state->fe_init = adap->fe->ops.init;
		adap->fe->ops.init = mxl111sf_adap_fe_init;
		adap_state->fe_sleep = adap->fe->ops.sleep;
		adap->fe->ops.sleep = mxl111sf_adap_fe_sleep;
...
static int mxl111sf_set_ant_path(struct mxl111sf_state *state, int antpath)
{
	...
	mxl111sf_idac_config(state, 1, 1, 0x00, 0); 
	
	mxl111sf_idac_config(state, 1, 1, 0x3f, 0); 
#endif
}

#define DbgAntHunt(x, pwr0, pwr1, pwr2, pwr3) err("%s(%d) FINAL input set to %s rxPwr:%d|%d|%d|%d\n", __FUNCTION__, __LINE__, (ANT_PATH_EXTERNAL == x) ? "EXTERNAL" : "INTERNAL", pwr0, pwr1, pwr2, pwr3)
...
static int mxl111sf_ant_hunt(struct dvb_frontend *fe)
{
	...
	if (antctrl == ANT_PATH_AUTO) {
		...
		if (rxPwr1+ANT_EXT_TWEAK >= rxPwr2) {
			...
			mxl111sf_set_ant_path(state, ANT_PATH_EXTERNAL);
			DbgAntHunt(ANT_PATH_EXTERNAL, rxPwrA, rxPwr0, rxPwr1, rxPwr2);
		} else {
			
			DbgAntHunt(ANT_PATH_INTERNAL, rxPwrA, rxPwr0, rxPwr1, rxPwr2);
		}
	}
	return 0;
}
...
static int mxl111sf_attach_tuner(struct dvb_usb_adapter *adap)
{
	...
	if (NULL != dvb_attach(mxl111sf_tuner_attach, adap->fe, state,
			       &mxl_tuner_config))
		return 0;
			  ...
			  const struct usb_device_id *id)
{
	...
	if (((dvb_usb_mxl111sf_isoc) &&
	     (0 == dvb_usb_device_init(intf, &mxl111sf_dvbt_isoc_properties,
				       THIS_MODULE, &d, adapter_nr) ||
	      0 == dvb_usb_device_init(intf, &mxl111sf_atsc_isoc_properties,
				       THIS_MODULE, &d, adapter_nr) ||
	      0 == dvb_usb_device_init(intf, &mxl111sf_atsc_mh_isoc_properties,
				       THIS_MODULE, &d, adapter_nr) ||
	      0 == dvb_usb_device_init(intf, &mxl111sf_mh_isoc_properties,
				       THIS_MODULE, &d, adapter_nr) ||
	      0 == dvb_usb_device_init(intf, &mxl111sf_mercury_spi_isoc_properties,
				       THIS_MODULE, &d, adapter_nr) ||
	      0 == dvb_usb_device_init(intf, &mxl111sf_mercury_tp_isoc_properties,
				       THIS_MODULE, &d, adapter_nr) ||
	      0 == dvb_usb_device_init(intf, &mxl111sf_mercury_mh_tp_isoc_properties,
				       THIS_MODULE, &d, adapter_nr))) ||
	    0 == dvb_usb_device_init(intf, &mxl111sf_dvbt_bulk_properties,
				     THIS_MODULE, &d, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, &mxl111sf_atsc_bulk_properties,
				     THIS_MODULE, &d, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, &mxl111sf_atsc_mh_bulk_properties,
				     THIS_MODULE, &d, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, &mxl111sf_mh_bulk_properties,
				     THIS_MODULE, &d, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, &mxl111sf_mercury_spi_bulk_properties,
				     THIS_MODULE, &d, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, &mxl111sf_mercury_tp_bulk_properties,
				     THIS_MODULE, &d, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, &mxl111sf_mercury_mh_tp_bulk_properties,
				     THIS_MODULE, &d, adapter_nr) || 0)
	{
		...
		err("MXL111SF HCW SRC buildno 29009\n");
...
static struct dvb_usb_device_properties mxl111sf_mercury_spi_bulk_properties = {
	...
	.num_device_descs = 1,
	.devices = {
		{   "HCW 138xxx (mercury-spi) ATSC+",
			{ NULL },
			{ &mxl111sf_table[19], &mxl111sf_table[20],
			  &mxl111sf_table[21], &mxl111sf_table[22],
			  &mxl111sf_table[23], &mxl111sf_table[24],
			  &mxl111sf_table[25], &mxl111sf_table[26],
			  NULL },
		},
	}
};
...
{
	int result;
	if ((result = usb_register(&mxl111sf_driver))) {
