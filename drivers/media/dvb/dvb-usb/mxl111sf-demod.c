...
#define mxl_dbg(fmt, arg...) \
	if (mxl111sf_demod_debug) mxl_printk(KERN_DEBUG, fmt, ##arg)
...
static int mxl111sf_demod_read_reg(struct mxl111sf_demod_state *state,
				   u8 addr, u8 *data)
{
	return (state->cfg->read_reg) ? state->cfg->read_reg(state->mxl_state, addr, data) :
#if 0
					mxl111sf_read_reg(state->mxl_state, addr, data);
#else
					-EINVAL;
#endif
}
...
static int mxl111sf_demod_write_reg(struct mxl111sf_demod_state *state,
				    u8 addr, u8 data)
{
	return (state->cfg->write_reg) ? state->cfg->write_reg(state->mxl_state, addr, data) :
#if 0
					mxl111sf_write_reg(state->mxl_state, addr, data);
#else
					-EINVAL;
#endif
}

static int mxl111sf_demod_program_regs(struct mxl111sf_demod_state *state,
			       struct mxl111sf_reg_ctrl_info *ctrl_reg_info)
{
	return (state->cfg->program_regs) ?
		state->cfg->program_regs(state->mxl_state, ctrl_reg_info) :
#if 0
		mxl111sf_ctrl_program_regs(state->mxl_state, ctrl_reg_info);
#else
		-EINVAL;
#endif
}
...
static int mxl1x1sf_demod_get_tps_code_rate(struct mxl111sf_demod_state *state,
...
static int mxl1x1sf_demod_get_tps_constellation(struct mxl111sf_demod_state *state,
...
static int mxl1x1sf_demod_get_tps_hierarchy(struct mxl111sf_demod_state *state,
...
#if 0
static int mxl1x1sf_demod_get_cp_lock_status(struct mxl111sf_demod_state *state,
					     int *cp_lock)
{
	u8 val = 0;
...
static int mxl111sf_demod_get_tune_settings(struct dvb_frontend *fe,
