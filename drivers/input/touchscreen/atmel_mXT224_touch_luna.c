...
#include <atmel_mXT224_touch_luna.h>
...
#define ATMEL_DRIVER_NAME "atmel_mXT224_touchscreen"
#define ATMEL_TOUCH_DEVICE_GUID NV_ODM_GUID('a','t','m','t','o','u','c','h')
...
static int __devinit touchpad_probe(struct platform_device *pdev );
                        ...
                        uint8_t           dataLen )
{
    ...
    TransactionInfo.Address = g_tp->DeviceAddr;
    ...
    TransactionInfo.Buf = (void *)buf;
    TransactionInfo.NumBytes = dataLen+sizeof(regBuf);
	
    while( retryCnt )
    {
		status = NvOdmI2cTransaction(g_tp->hOdmI2c, &TransactionInfo, 1, 
    ...
    if( (result == 0) && (retryCnt < TOUCH_RETRY_COUNT) )
        MY_INFO_PRINTK( 4, "INFO_LEVEL:""write 0x%X 0x%X %d bytes retry at %d\n", buf[0], buf[1], dataLen, TOUCH_RETRY_COUNT-retryCnt);
                        ...
                        uint8_t           dataLen )
{
	...
	int     retryCnt_1 = TOUCH_RETRY_COUNT;
    NvOdmI2cStatus status = NvOdmI2cStatus_Success;    
    NvOdmI2cTransactionInfo TransactionInfo[2];
 
	
	TransactionInfo[0].Address = g_tp->DeviceAddr;
	TransactionInfo[0].Buf = (void *)&regBuf;
	TransactionInfo[0].Flags = NVODM_I2C_IS_WRITE;
	TransactionInfo[0].NumBytes = sizeof(regBuf);
	while( retryCnt_0 )
    {
		status = NvOdmI2cTransaction(g_tp->hOdmI2c, &TransactionInfo[0], 1, 
										100, NV_WAIT_INFINITE);
		if (status != NvOdmI2cStatus_Success)
		{
			switch (status)
	...
	if( (retryCnt_0 < TOUCH_RETRY_COUNT) )
        MY_INFO_PRINTK( 4, "INFO_LEVEL:""read %Xh %d bytes retry at %d\n", regBuf, dataLen, TOUCH_RETRY_COUNT-retryCnt_0 );
		
	TransactionInfo[1].Address = ((g_tp->DeviceAddr) | 0x1);
	TransactionInfo[1].Buf = (void *)dataBuf;
	TransactionInfo[1].Flags = 0;
	TransactionInfo[1].NumBytes = dataLen;
	while( retryCnt_1 )
    {
		
		status = NvOdmI2cTransaction(g_tp->hOdmI2c, &TransactionInfo[1], 1, 
										100, NV_WAIT_INFINITE);
		if (status != NvOdmI2cStatus_Success)
		{
			switch (status)
    ...
    if( (retryCnt_1 < TOUCH_RETRY_COUNT) )
        MY_INFO_PRINTK( 4, "INFO_LEVEL:""read %Xh %d bytes retry at %d\n", regBuf, dataLen, TOUCH_RETRY_COUNT-retryCnt_1 );
	if(status != NvOdmI2cStatus_Success)
	{
                            ...
                            uint8_t           dataLen )
{
	...
	result = touchpad_read_i2c(NULL,addr,dataBuf,dataLen);
	if(result)
	{
...
static int calculate_infoblock_crc(struct touchpad_t *g_tp)
{
    ...
    uint8_t  i;
   	...
   	result = touchpad_read_i2c( NULL,crc_area_size, &value[0], INFO_BLOCK_CHECKSUM_SIZE);
   	...
   	result = touchpad_read_i2c( NULL,ADD_ID_INFO_BLOCK, &value[0], crc_area_size);
...
static void tp_report_coord_via_mt_protocol(void)
{
	...
	for(i=0;i<ATMEL_REPORT_POINTS;i++)
	{
		...
		else
		{
			...
			MY_INFO_PRINTK( 1,"INFO_LEVEL:" "x %d y %d",g_tp->msg[i].coord.x,g_tp->msg[i].coord.y ); 
		}
...
static void touchpad_irqWorkHandler( struct work_struct *work )
{
    ...
    if(!rt_task(current))
    {
		...
		{
			MY_INFO_PRINTK( 1, "TEST_INFO_LEVEL:" "fail to set rt pri...\n" );
		}
		else
		{
			MY_INFO_PRINTK( 1, "TEST_INFO_LEVEL:" "set rt pri...\n" );
		}
    }
	...
	NvOdmGpioInterruptDone(g_tp->hGpioIntr);
	mutex_unlock(&g_tp->mutex);
	PRINT_OUT
    return;
}

static void touchpad_irqHandler(void *arg)
{
    PRINT_IN
...
static int touchpad_poweron_device(struct touchpad_t *g_tp, NvBool OnOff)
{
    int     rc = 0;

    PRINT_IN
    MY_INFO_PRINTK( 4,"INFO_LEVEL:""power switch %d\n", OnOff );
	g_tp->hPmu = NvOdmServicesPmuOpen();
	
	if (!g_tp->hPmu)
    {
        MY_INFO_PRINTK( 0,"ERROR_LEVEL:""NvOdmServicesPmuOpen Error \n");
        return -EFAULT;
    }
    
    if (OnOff != g_tp->PowerOn)
    {
        NvOdmServicesPmuVddRailCapabilities vddrailcap;
        NvU32 settletime;

        NvOdmServicesPmuGetCapabilities( g_tp->hPmu, g_tp->VddId, &vddrailcap);

        if(OnOff)
		{
            NvOdmServicesPmuSetVoltage( g_tp->hPmu, g_tp->VddId, vddrailcap.requestMilliVolts, &settletime);
        ...
        if (settletime)
		 ...
		 if(OnOff)
		{
			NvOdmServicesPmuSetVoltage( g_tp->hPmu, g_tp->AVddId, vddrailcap.requestMilliVolts, &settletime);
			MY_INFO_PRINTK( 4,"INFO_LEVEL:""power_switch_AVDD(%d)\n", g_tp->AVddId);
			MY_INFO_PRINTK( 4,"INFO_LEVEL:""power_switch_AVDD_Voltage(%d)\n", vddrailcap.requestMilliVolts);
		}
        else
		{
        ...
        if(OnOff)
            NvOdmOsSleepMS(ATMEL_POR_DELAY);
    }

    NvOdmServicesPmuClose(g_tp->hPmu);
...
static int touchpad_setup_gpio( struct touchpad_t *g_tp )
{
	...
	NvOdmGpioConfig(g_tp->hGpio,g_tp->hIntPin,NvOdmGpioPinMode_InputInterruptLow);
	
	MY_INFO_PRINTK( 4, "INFO_LEVEL:""setup gpio Rstpin \n"); 
	NvOdmGpioConfig(g_tp->hGpio,g_tp->hRstPin,NvOdmGpioPinMode_Output);
	PRINT_OUT
    return rc;
}
...
static int touchpad_config_gpio( struct touchpad_t *g_tp )
{
	...
	NvOdmGpioSetState(g_tp->hGpio, g_tp->hRstPin, 0);
	MY_INFO_PRINTK( 4,"INFO_LEVEL:""touchpad_config_gpio reset get low : delay 1 ms\n");
    msleep(1);
	
	
    NvOdmGpioSetState(g_tp->hGpio, g_tp->hRstPin, 1);
    MY_INFO_PRINTK( 4,"INFO_LEVEL:""touchpad_config_gpio reset get high : delay 40 ms\n");
...
static int touchpad_config_mXT224(struct touchpad_t *g_tp)
{
    ...
    for ( i = 0 ; i < g_tp->id_info.num_obj_element ; i++ )
    {
    	...
    	{
			result = touchpad_write_i2c(NULL,g_tp->init_obj_element[i]->obj_addr,g_tp->init_obj_element[i]->value_array,g_tp->init_obj_element[i]->size);
    ...
    result = touchpad_write_i2c(NULL,regbuf,&value,WRITE_T6_SIZE);
    ...
    result = touchpad_write_i2c(NULL,maxTouchCfg_T6_obj.obj_addr,&value ,WRITE_T6_SIZE);
...
static int boot_write_mem(uint16_t start, uint16_t size, uint8_t *mem)
{
    ...
    TransactionInfo.Address = (0x24 << 1);
    TransactionInfo.Buf = mem;
    TransactionInfo.Flags = NVODM_I2C_IS_WRITE;
    TransactionInfo.NumBytes = size;
...
static int boot_read_mem(uint16_t start, uint8_t size, uint8_t *mem)
{
    ...
    int     retryCnt_0 = TOUCH_RETRY_COUNT;
   NvOdmI2cStatus status = NvOdmI2cStatus_Success;    
    NvOdmI2cTransactionInfo TransactionInfo[1];
 
	
	TransactionInfo[0].Address = ((0x24<<1)|0x01);
	TransactionInfo[0].Buf = mem;
	TransactionInfo[0].Flags = 0;
	TransactionInfo[0].NumBytes = size;
	while( retryCnt_0 )
    {
		status = NvOdmI2cTransaction(g_tp->hOdmI2c, &TransactionInfo[0], 1, 
										100, NV_WAIT_INFINITE);
		if (status != NvOdmI2cStatus_Success)
		{
			switch (status)
			{
				case NvOdmI2cStatus_Timeout:
				 ...
				 case NvOdmI2cStatus_SlaveNotFound:
				 default:
					MY_INFO_PRINTK( 0, "ERROR_LEVEL:""NvOdmPmuI2cRead8 Failed: SlaveNotFound\n" );
					break;             
			}
			retryCnt_0--;
		}         
...
{	
	...
	value = 0xA5;;
    result = touchpad_write_i2c(NULL,maxTouchCfg_T6_obj.obj_addr,&value ,1);
...
static uint8_t write_mem(uint16_t start, uint8_t size, uint8_t *mem)
{
	...
	ret = touchpad_write_i2c(NULL,start,mem,size);
	if(ret) 
...
detect_again:
	...
	result = touchpad_read_i2c( NULL, ADD_ID_INFO_BLOCK, &value[0],NUM_OF_ID_INFO_BLOCK );
...
static int power_on_flow_another_way(struct touchpad_t *g_tp)
{
	...
	else
	{
		...
		MY_INFO_PRINTK( 0, "ERROR_LEVEL:" "touchpad_probe: failed to get hIntPin %p pinvalue %d\n",g_tp->hIntPin, pinValue );
        PRINT_OUT
        return result;
	}
    ...
    for (;;)
	{
		touchpad_read_i2c( NULL, maxTouchCfg_T5_obj.obj_addr, &value[0],obj_size ); 
    	i = 0;
...
static void tp_suspend(struct early_suspend *h)
{
    ...
    if (ret) 
    {
		NvOdmGpioInterruptMask(g_tp->hGpioIntr, 0); 
		MY_INFO_PRINTK( 4, "INFO_LEVEL:" "enalbe irq %p\n",g_tp->hGpioIntr );
	}	
...
static struct platform_driver atmel_touch_driver = {
	.driver	 = {
                                    ...
                                    struct platform_device *pdev )
{
    ...
    input_set_abs_params(input_dev, ABS_Y, 0, ATMEL_TOUCAN_PANEL_Y, 0, 0);
...
static int query_touch_pin_info(struct touchpad_t* g_tp)
{
    ...
    PRINT_IN
    pConnectivity = NvOdmPeripheralGetGuid(ATMEL_TOUCH_DEVICE_GUID);
    if (!pConnectivity)
    {
        MY_INFO_PRINTK( 0, "ERROR_LEVEL:" "NvOdm Touch : pConnectivity is NULL Error \n");
        goto fail;
    }

    if (pConnectivity->Class != NvOdmPeripheralClass_HCI)
    {
        MY_INFO_PRINTK( 0, "ERROR_LEVEL:" "NvOdm Touch : didn't find any periperal in discovery query for touch device Error \n");
        goto fail;
    }
	...
	for (i = 0; i < pConnectivity->NumAddress; i++)
    {
        switch (pConnectivity->AddressList[i].Interface)
        {
            case NvOdmIoModule_I2c:
                g_tp->DeviceAddr = (pConnectivity->AddressList[i].Address << 1);
            ...
            case NvOdmIoModule_Vdd:
                if( i == 3)
					g_tp->VddId = pConnectivity->AddressList[i].Address;
				if (system_rev <= EVT1_2 || system_rev == HWID_UNKNOWN)
				{
					if(i == 4)
						g_tp->AVddId = pConnectivity->AddressList[i].Address;
    ...
    g_tp->hOdmI2c = NvOdmI2cOpen(NvOdmIoModule_I2c, I2cInstance);
	MY_INFO_PRINTK( 4, "INFO_LEVEL:""NvOdm Touch : NvOdmI2cAddr :%d \n",g_tp->DeviceAddr);
	MY_INFO_PRINTK( 4, "INFO_LEVEL:""NvOdm Touch : I2cInstance :%d \n",I2cInstance);
    if (!g_tp->hOdmI2c)
    {
        MY_INFO_PRINTK( 0, "ERROR_LEVEL:""NvOdm Touch : NvOdmI2cOpen Error \n");
        goto fail;
    }

    g_tp->hGpio = NvOdmGpioOpen();
    if (!g_tp->hGpio)
    {        
        MY_INFO_PRINTK( 0, "ERRIR_LEVEL:""NvOdm Touch : NvOdmGpioOpen Error \n");
        goto fail;
    }

    g_tp->hIntPin = NvOdmGpioAcquirePinHandle(g_tp->hGpio, IntGpioPort, IntGpioPin);
	MY_INFO_PRINTK( 4, "INFO_LEVEL:""NvOdm Touch : get GPIO Int_port %d GPIO Int_pin %d \n",IntGpioPort, IntGpioPin);
    if (!g_tp->hIntPin)
    {
        MY_INFO_PRINTK( 0, "ERROR_LEVEL:""NvOdm Touch : Couldn't get GPIO pin \n");
        goto fail;
    }

    g_tp->hRstPin = NvOdmGpioAcquirePinHandle(g_tp->hGpio, RstGpioPort, RstGpioPin);
	MY_INFO_PRINTK( 4, "INFO_LEVEL:""NvOdm Touch : get GPIO Rst_port %d GPIO Rst_pin %d \n",RstGpioPort, RstGpioPin);
    if (!g_tp->hRstPin)
    {
        MY_INFO_PRINTK( 0, "ERROR_LEVEL:""NvOdm Touch : Couldn't get GPIO pin \n");
        goto fail;
...
fail:;
    ...
    if (g_tp->hGpio)
    {
        if (g_tp->hIntPin)
        {
			...
			#endif
            NvOdmGpioReleasePinHandle(g_tp->hGpio, g_tp->hIntPin);
        }
		if (g_tp->hRstPin)
        {
            #if 0
			if (pTS->hGpioIntr)
                NvOdmGpioInterruptUnregister(hTouch->hGpio, hTouch->hPin, hTouch->hGpioIntr);
			#endif
            NvOdmGpioReleasePinHandle(g_tp->hGpio, g_tp->hRstPin);
        }
		
        NvOdmGpioClose(g_tp->hGpio);
    }

    if (g_tp->hOdmI2c)
        NvOdmI2cClose(g_tp->hOdmI2c);
...
static int __devinit touchpad_probe(struct platform_device *pdev )
{
	...
	platform_set_drvdata(pdev, g_tp);
    ...
    if( 0 == pinValue )
	{
		MY_INFO_PRINTK( 0, "INFO_LEVEL:" "touchpad_probe: get hIntPin %p pinvalue %d\n",g_tp->hIntPin, pinValue );
	}
	else
	{
		MY_INFO_PRINTK( 0, "ERROR_LEVEL:" "touchpad_probe: failed to get hIntPin %p pinvalue %d\n",g_tp->hIntPin, pinValue );
    ...
    g_tp->touchpad_wqueue = create_singlethread_workqueue(ATMEL_DRIVER_NAME);
	...
	if (NvOdmGpioInterruptRegister(g_tp->hGpio, &g_tp->hGpioIntr,
    g_tp->hIntPin, NvOdmGpioPinMode_InputInterruptLow, touchpad_irqHandler,
	(void*)g_tp, 0) == NV_FALSE)
	{
...
static int __init touchpad_init(void)
{
	...
	return platform_driver_register(&atmel_touch_driver);
    PRINT_OUT
    return rc;
}
...
{  
	PRINT_IN
	platform_driver_unregister(&atmel_touch_driver);
	#if 0
