...
static irqreturn_t tegra_ehci_remote_wakeup_irq(int irq, void *arg)
{
	return IRQ_HANDLED;
}
#endif
...
static int tegra_ehci_bus_suspend(struct usb_hcd *hcd)
{
	...
	if (ehci->host_resumed) {
		...
		if (!error_status)
			...
			printk(KERN_ERR "%s: ehci_bus_suspend fail\n", __func__);
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
		if (!error_status && pdata->instance == 2) {
			gpio_request(pdata->wakeup_nr, hcd->self.bus_name);
			enable_irq(pdata->wakeup_irq);
			set_irq_wake(pdata->wakeup_irq, 1);
		}
#endif
	}
...
static int tegra_ehci_bus_resume(struct usb_hcd *hcd)
{
...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
		if (pdata->instance == 2) {
			disable_irq(pdata->wakeup_irq);
			gpio_free(pdata->wakeup_nr);
		}
#endif
		tegra_ehci_power_up(hcd);
	}
	...
	tegra_usb_phy_preresume(hcd);
	error_status = ehci_bus_resume(hcd);
	tegra_usb_phy_postresume(hcd);
...
static int tegra_ehci_probe(struct platform_device *pdev)
{
	...
	if (pdata->instance == 2) {
		gpio_request(pdata->wakeup_nr, hcd->self.bus_name);
		gpio_direction_input(pdata->wakeup_nr);
		pdata->wakeup_irq = gpio_to_irq(pdata->wakeup_nr);
		request_irq(pdata->wakeup_irq, &tegra_ehci_remote_wakeup_irq,
			IRQF_TRIGGER_FALLING, dev_name(&pdev->dev), pdata);
		disable_irq(pdata->wakeup_irq);
		gpio_free(pdata->wakeup_nr);
	}
#endif
