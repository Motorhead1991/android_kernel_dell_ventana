...
static int IN_BUFLEN = 4096;
static int OUT_BUFLEN = 4096;
...
struct wake_lock option_wake_lock;
struct wake_lock option_discon_lock;

#define OPTION_LOCK(lock)  \
{ \
	if (!wake_lock_active(&lock)) \
	{ \
		wake_lock(&lock); \
		printk(KERN_INFO "%s: %s locked!\n", __func__, lock.name); \
	} \
} 

#define OPTION_LOCK_TIMEOUT(lock, sec)  \
{ \
	if (!wake_lock_active(&lock)) \
	{ \
...
#define OPTION_UNLOCK(lock) \
{ \
	if (wake_lock_active(&lock)) \
	{ \
		wake_unlock(&lock); \
		printk(KERN_INFO "%s: %s unlocked!\n", __func__, lock.name); \
	} \
}
...
static void option_open_work(struct work_struct *work);
static void option_close_work(struct work_struct *work);
int  option_bridge_copyto_us (unsigned int serial_index, char *packet, unsigned int size);
void option_start_read_from_us(void);
void option_stop_read_from_us(void);

#define MAX_SERIAL_ARRAY 0xf
static struct usb_serial *option_for_huawei[MAX_SERIAL_ARRAY] = {0};
static struct usb_serial *huawei_diag = NULL;

static int option_serial_in_array(struct usb_serial *serial)
{
	int index = 0;
	for (index = 0; index < MAX_SERIAL_ARRAY; index++) {
		if (option_for_huawei[index] == serial) {
			break;
		}
	}
	return index;
}
static void option_destroy_bridge_data(struct usb_serial *serial)
{
	int index = 0;
	for (index = 0; index < MAX_SERIAL_ARRAY; index++) {
		if (option_for_huawei[index] == serial) {
	...
	for (index = 0; index < MAX_SERIAL_ARRAY; index++) {
		option_for_huawei[index] = NULL;
	}
	gadgetutil.bridge_copyto_us = NULL;
	gadgetutil.bridge_start_read_from_us = NULL;
	gadgetutil.bridge_stop_read_from_us = NULL;
	cancel_work_sync(&open_work);
	cancel_work_sync(&close_work);
	bridge_mode = 0;
	product_id = 0x1001;
}
struct usb_serial_port *opening_port = NULL;
...
static void option_init_bridge_data(struct usb_serial *serial)
{
	if (NULL == serial) {
		return;
	}
	if (HUAWEI_VENDOR_ID == serial->dev->descriptor.idVendor) {
		int index = 0;
		if (NULL == option_for_huawei[0]) {
			option_for_huawei[0] = serial;
		} else {
			if (serial->dev != option_for_huawei[0]->dev) {
				for (index = 0; index < MAX_SERIAL_ARRAY; index++) {
					option_for_huawei[index] = NULL;
				}
				option_for_huawei[0] = serial;
				...
				index = 0;
				while (NULL != option_for_huawei[index]){
					index++;
					if (index >= MAX_SERIAL_ARRAY) {
						break;
					}
				...
				if (index < MAX_SERIAL_ARRAY) {
					option_for_huawei[index] = serial;
				}
			}
		}
		product_id = serial->dev->descriptor.idProduct;
		INIT_WORK(&open_work, option_open_work);
		INIT_WORK(&close_work, option_close_work);
		gadgetutil.bridge_copyto_us = option_bridge_copyto_us;
		gadgetutil.bridge_start_read_from_us = option_start_read_from_us;
		gadgetutil.bridge_stop_read_from_us = option_stop_read_from_us;
...
int option_bridge_copyto_us (unsigned int serial_index, char *packet, unsigned int size)
{
	...
	opening_port = huawei_diag->port[0];
...
static void option_open_work(struct work_struct *work)
{
	...
	int index = 0;
    if (NULL != opening_port) {
		portdata = usb_get_serial_port_data(opening_port);
		portdata->dtr_state = 0x01;
		portdata->rts_state = 0x02;
		option_open(NULL, opening_port);
		for (index = 0; index < 32; index++) {
			if (0 < gs_bridge_buffer[index].len) {
				option_write(NULL, opening_port, gs_bridge_buffer[index].szbuf, gs_bridge_buffer[index].len);
...
static void option_close_work(struct work_struct *work)
{
	...
	for (idx = 0; idx < 3; idx++) {
		index = gadget_get_map_index_bridge_to_device(option_for_huawei[0]->dev->descriptor.idProduct, idx);
		if (0xf != index && NULL != option_for_huawei[index]
			&& NULL != option_for_huawei[index]->port[0]) {
			option_close(option_for_huawei[index]->port[0]);
		}
	}
	#endif
}
...
void option_copyto_gs(struct usb_serial *serial, struct urb *urb) 
{
	int index = -1;
	unsigned char *data;
	if (NULL == serial || NULL == urb) {
		return;
	}
	data = urb->transfer_buffer;
	index = gadget_get_map_index_device_to_bridge(serial->dev->descriptor.idProduct,
		option_serial_in_array(serial));
	
	if (NULL != gadgetutil.bridge_copyto_gs && urb->actual_length){
		gadgetutil.bridge_copyto_gs(index, data, urb->actual_length);
	}
	return;
}
...
static int __init option_init(void)
{
	...
	wake_lock_init(&option_wake_lock, WAKE_LOCK_SUSPEND, "option_wake_lock");
	wake_lock_init(&option_discon_lock, WAKE_LOCK_SUSPEND, "option_discon_lock");
...
static void __exit option_exit(void)
{
	...
	wake_lock_destroy(&option_wake_lock);
	wake_lock_destroy(&option_discon_lock);
}

module_init(option_init);
module_exit(option_exit);

extern void tegra_set_wakeup_pad(int entry, int isEnable);
			...
			const struct usb_device_id *id)
{
	...
	if (bridge_enabled)
	{
		...
		if (HUAWEI_VENDOR_ID == serial->dev->descriptor.idVendor) {
			product_id = serial->dev->descriptor.idProduct;
			diag = gadget_get_diag_intfnum(product_id);
			printk(KERN_ERR"fxz-%s: diag[%d], index[%d]\n", __func__, diag, serial->interface->cur_altsetting->desc.bInterfaceNumber);
			if (0xf != diag && diag == serial->interface->cur_altsetting->desc.bInterfaceNumber) {
				printk(KERN_ERR"fxz-%s: put huawei_diag\n", __func__);
				huawei_diag = serial;
			}
		}
		option_init_bridge_data(serial);
	}
...
static void option_indat_callback(struct urb *urb)
{
	...
	if (bridge_enabled) {
		if (status) {
			...
			if (huawei_diag == port->serial) {
				if (urb->actual_length) {
					option_copyto_gs(port->serial, urb);
				}
			} else {
		...
		if (err) {
			if(0 != bridge_mode) {
				(void)usb_submit_urb(urb, GFP_ATOMIC);
			}
...
static int option_open(struct tty_struct *tty, struct usb_serial_port *port)
{
	...
	if (bridge_enabled && (portdata->opened == 1))
		return 0;
...
static void option_disconnect(struct usb_serial *serial)
{
	...
	OPTION_LOCK_TIMEOUT(option_discon_lock, 10);	

	
	if(hw_udev && hw_udev == serial->dev) {
		if (hw_suspend_wq) {
			cancel_delayed_work_sync(hw_suspend_wq); 
			OPTION_UNLOCK(option_wake_lock);
		}
		hw_udev = NULL;
	}
	...
	if (bridge_enabled)
	{
		...
		option_destroy_bridge_data(serial);
	}
...
static void option_suspend_check_work(struct work_struct *work)
{
	...
	if (current_time > hw_udev->last_busy + (suspend_check * HZ)/1000) {
#if 0
