...
static int misc_current_driving_open(struct inode *inode_p, struct file *fp)
{
	...
	{
		if (!current_driving_driver_data.RmDeviceHandle)
			NvRmOpen(&(current_driving_driver_data.RmDeviceHandle), 0);
...
static int misc_current_driving_release(struct inode *inode_p, struct file *fp)
{
	...
	{
		if (driver_data->RmDeviceHandle)
			NvRmClose(driver_data->RmDeviceHandle);
		driver_data->RmDeviceHandle = NULL;
	}
...
static long misc_current_driving_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case CURRENT_DRIVING_IOC_WRITE:
			...
			NV_REGW(driver_data->RmDeviceHandle, NvRmModuleID_Misc, 0, current_driving_ctrl.padctrl,
				NVODM_QUERY_PIN_AP20_PADCTRL_AOCFG1PADCTRL(!HIGHSPEED, SCHMITT, current_driving_ctrl.ohm, 31, 31, 3, 3));
			break;
