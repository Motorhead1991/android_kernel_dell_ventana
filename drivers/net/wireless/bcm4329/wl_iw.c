...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		if (wl_iw_busdown&&!wl_iw_recover) {
			...
			printk("%s: restart processing. skip it.\n", __FUNCTION__);
			return -ENODEV;
		}
#endif
...
wl_fixed_scan_prep(wl_scan_params_t *params, int *params_size)
{
    ...
    memset(ssids, 0, 10 * sizeof(wlc_ssid_t));
...
wl_iw_fixed_scan(struct net_device *dev)
{
    ...
    printk("%s:\n", __func__);
    params_size += 10 * sizeof(wlc_ssid_t);
...
)
{
	...
	if ((error = dev_wlc_ioctl(dev, WLC_SCAN_RESULTS, list, len))) {
		WL_ERROR(("%s: %s : Scan_results ERROR %d\n", dev->name, __FUNCTION__, error));
                if(! wl_iw_fixed_scan(dev)) 
                    printk("*************** Fail to recover the scan\n"); 
                else
                {
                    printk("***********Successful recover scan hang***********\n"); 
                }
		dwrq->length = len;
		if (g_scan_specified_ssid) {
			g_scan_specified_ssid = 0;
