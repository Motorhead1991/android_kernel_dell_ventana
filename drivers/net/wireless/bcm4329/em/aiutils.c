...
STATIC uint32
BCMROMFN(get_asd)(si_t *sih, uint32 *eromptr, uint sp, uint ad, uint st,
...
void *
BCMROMFN(ai_setcoreidx)(si_t *sih, uint coreidx)
{
	si_info_t *sii = SI_INFO(sih);
	...
	case SPI_BUS:
	case SDIO_BUS:
#if defined(BCMJTAG) || defined(BCMROMBUILD)
	case JTAG_BUS:
#endif	
...
uint
BCMROMFN(ai_corereg)(si_t *sih, uint coreidx, uint regoff, uint mask, uint val)
{
	uint origidx = 0;
	uint32 *r = NULL;
	uint w;
...
void
BCMROMFN(ai_core_disable)(si_t *sih, uint32 bits)
{
	si_info_t *sii;
...
void
BCMROMFN(ai_core_reset)(si_t *sih, uint32 bits, uint32 resetbits)
{
	si_info_t *sii;
	aidmp_t *ai;
	volatile uint32 dummy;
...
uint32
BCMROMFN(ai_core_cflags)(si_t *sih, uint32 mask, uint32 val)
{
	si_info_t *sii;
	aidmp_t *ai;
	uint32 w;
...
uint32
BCMROMFN(ai_core_sflags)(si_t *sih, uint32 mask, uint32 val)
{
	si_info_t *sii;
	aidmp_t *ai;
	uint32 w;
