...
extern char wifi_mac[wifi_mac_size];   
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
        ...
        sprintf(tmp_mac, "%c%c:%c%c:%c%c:%c%c:%c%c:%c%c", wifi_mac[0], wifi_mac[1],
                            wifi_mac[2], wifi_mac[3], wifi_mac[4], wifi_mac[5],
                            wifi_mac[6], wifi_mac[7], wifi_mac[8], wifi_mac[9],
                            wifi_mac[10], wifi_mac[11]);

        i=sscanf(tmp_mac, "%x:%x:%x:%x:%x:%x", &o0, &o1, &o2, &o3, &o4, &o5);
        printk("Reading WLAN MAC: %s\n", tmp_mac);
    
        if((i == 6) && (strncmp(tmp_mac,"00:00:00:00:00:00",17)!=0))
        {
        ...
        {
           printk("#########Failed of getting valid MAC\n");
...
int dhd_keep_alive_onoff(dhd_pub_t *dhd, int ka_on)
{
	...
	int str_len, buf_len;
	int res = 0;
	int keep_alive_period = KEEP_ALIVE_PERIOD; 
