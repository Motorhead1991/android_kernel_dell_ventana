...
#define GSENSOR_GUID (NV_ODM_GUID('g','_','b','m','a','1','5','0'))
#define GSENSOR_EVT1B_GUID (NV_ODM_GUID('b','m','a','1','5','0','1','b'))
#define GSENSOR_NAME "gsensor_bma150"
#define GSENSOR_I2C_NAME "gsensor_bma150_i2c"
#define GSENSOR_I2C_TIMEOUT 3000 
#define GSENSOR_I2C_SCLK  100
...
static int i2c_gsensor_probe(struct platform_device *pdev);
static int i2c_gsensor_remove(struct platform_device *pdev);
static int i2c_gsensor_suspend(struct platform_device *pdev, pm_message_t state);
static int i2c_gsensor_resume(struct platform_device *pdev);
...
static int i2c_gsensor_read(NvOdmServicesI2cHandle hI2C, uint8_t regaddr, uint8_t *buf, uint32_t len)
{
	uint8_t ldat = regaddr;
	NvOdmI2cStatus I2cTransStatus = -NvOdmI2cStatus_Timeout;
	NvOdmI2cTransactionInfo TransactionInfo[] = {
		[0] = {
			.Address	= gsensor_driver_data.I2CAddr,
	...
	I2cTransStatus = NvOdmI2cTransaction(hI2C, &TransactionInfo[0], 2, GSENSOR_I2C_SCLK, GSENSOR_I2C_TIMEOUT);
	if (I2cTransStatus != NvOdmI2cStatus_Success)
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::NvOdmI2cTransaction err\n", __func__);

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] %s-ret:%d\n", __func__, -I2cTransStatus);
	return -I2cTransStatus;
}

static int i2c_gsensor_write(NvOdmServicesI2cHandle hI2C, uint8_t regaddr, uint8_t *buf, uint32_t len)
{
	...
	write_buf[0] = regaddr;
	NvOdmOsMemcpy((void *)(write_buf + 1), (void *) buf, len);

	I2cTransStatus = NvOdmI2cTransaction(hI2C, &TransactionInfo[0], 1, GSENSOR_I2C_SCLK, GSENSOR_I2C_TIMEOUT);
	if (I2cTransStatus != NvOdmI2cStatus_Success)
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::NvOdmI2cTransaction err\n", __func__);
...
static void debug_monitor_work_func(struct work_struct *work)
{
	...
	i2c_gsensor_read(driver_data->hI2C, BMA150_REG_ACC_X_LSB, rx_buf, sizeof(rx_buf));
...
static int gsensor_power_down(struct gsensor_driver_data_t *gsensor_driver_data)
{
	...
	ret = i2c_gsensor_write(gsensor_driver_data->hI2C, BMA150_REG_SLEEP, &data, sizeof(data));
	if (ret >= 0)
		ret = 0;
	else
	{
...
static int gsensor_set_ctrl_0b(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_RESET_INT, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_CTRL_0B, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_CTRL_0B, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
...
static int gsensor_set_settings_0c(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_LG_THRES, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_LG_THRES, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_settings_0d(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_LG_DUR, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_LG_DUR, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_settings_0e(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_HG_THRES, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_HG_THRES, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_settings_0f(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_HG_DUR, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_HG_DUR, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_settings_10(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_ANY_MOTION_THRES, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_ANY_MOTION_THRES, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_settings_11(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_ANY_MOTION_DUR, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_ANY_MOTION_DUR, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_ctrl_14(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_BANDWIDTH, &rx_data, sizeof(rx_data));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
		return ret;
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_BANDWIDTH, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_ctrl_15(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_setting_t *ioctl_setting)
{
	...
	do
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_ENABLE_ADV_INT, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_ENABLE_ADV_INT, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
...
static int gsensor_set_eeprom_write(struct gsensor_driver_data_t *driver_data, int enable_eeprom_write)
{
	...
	do
	{
		...
		if (enable_eeprom_write)
			...
			tx_data = 0x0;
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_EE_W, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return ret;
		...
		ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_EE_W, &rx_data, sizeof(rx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
			return ret;
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] gsensor_set_eeprom_write-\n");
	return 0;
}
...
static int gsensor_default_config(struct gsensor_driver_data_t *gsensor_driver_data)
{
	...
	ret = i2c_gsensor_read(gsensor_driver_data->hI2C, BMA150_REG_CHIP_ID, chipid, sizeof(chipid));
	if (ret < 0)
	{
	...
	ret = i2c_gsensor_read(gsensor_driver_data->hI2C, BMA150_REG_BANDWIDTH, bandwidth, sizeof(bandwidth));
	if (ret < 0)
	{
	...
	for (i = 0; i < 4; i++)
	{
		TransactionInfo[i].Address = gsensor_driver_data->I2CAddr,
		TransactionInfo[i].Flags	= NVODM_I2C_IS_WRITE,
		TransactionInfo[i].Buf = (NvU8 *) (buf + (i << 1)),
		TransactionInfo[i].NumBytes = 2,
		buf[(i << 1)] = reg[i];
		buf[(i << 1) + 1] = reg_data[i];
	}

	I2cTransStatus = NvOdmI2cTransaction(gsensor_driver_data->hI2C, &TransactionInfo[0], sizeof(TransactionInfo) / sizeof(TransactionInfo[0]), GSENSOR_I2C_SCLK, GSENSOR_I2C_TIMEOUT);
	if (I2cTransStatus != NvOdmI2cStatus_Success)
	{
...
static int gsensor_calibrate_config(struct gsensor_driver_data_t *driver_data)
{
	...
	ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_CTRL_15, &rx_data, sizeof(rx_data));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
		return ret;
	...
	tx_data = rx_data;
	ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_CTRL_15, &tx_data, sizeof(tx_data));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
		return ret;
	...
	ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_CTRL_14, &rx_data, sizeof(rx_data));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
		return ret;
	...
	tx_data = rx_data;
	ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_CTRL_14, &tx_data, sizeof(tx_data));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
		return ret;
...
static void gsensor_get_offset(struct gsensor_driver_data_t *driver_data, unsigned short *offset)
{
	...
	NvOdmOsMemset(rx_buf, 0, sizeof(rx_buf));
	ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_OFFSET_X_LSB, rx_buf, sizeof(rx_buf));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
		return;
...
static void gsensor_set_offset(struct gsensor_driver_data_t *driver_data, unsigned short *offset)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_OFFSET_X_LSB + i, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return;
	...
	for (i = 0; i < 3; i++)
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_OFFSET_X_MSB + i, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return;
...
static void gsensor_set_offset_eeprom(struct gsensor_driver_data_t *driver_data, unsigned short *offset)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_EEP_OFFSET_X_LSB + i, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return;
	...
	for (i = 0; i < 3; i++)
	{
		...
		ret = i2c_gsensor_write(driver_data->hI2C, BMA150_REG_EEP_OFFSET_X_MSB + i, &tx_data, sizeof(tx_data));
		if (ret < 0)
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
			return;
...
static int gsensor_read_accel_xyz(struct gsensor_driver_data_t *driver_data, struct orientation_accel_t *accel)
{
	...
	ret = i2c_gsensor_read(driver_data->hI2C, BMA150_REG_ACC_X_LSB, rx_buf, sizeof(rx_buf));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_read fail!!-ret:%d\n", __func__, ret);
		return 0;
...
static int gsensor_read_accel_avg(struct gsensor_driver_data_t *driver_data, int num_avg, struct orientation_accel_t *min, struct orientation_accel_t *max, struct orientation_accel_t *avg )
{
	...
	NvOdmOsMemset(&accel, 0, sizeof(struct orientation_accel_t));
	for (i=0; i<num_avg; i++) {
...
static int gsensor_engineer_mode_get_accel(struct gsensor_driver_data_t *driver_data, struct gsensor_ioctl_accel_t *ioctl_accel)
{
	int ret = 0;
	uint8_t rx_buf[6];
	NvOdmGpioPinActiveState gpiovalue = NvOdmGpioPinActiveState_Low;

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] %s+\n", __func__);

	if (driver_data->hGpio && driver_data->hIntrGpioPin)
                NvOdmGpioGetState(driver_data->hGpio, driver_data->hIntrGpioPin, (NvU32 *)&gpiovalue);
	ioctl_accel->interrupt = gpiovalue;
...
static void gsensor_irq_handler(void* arg)
{
	struct gsensor_driver_data_t *driver_data = (struct gsensor_driver_data_t *)arg;
...
static int misc_gsensor_open(struct inode *inode_p, struct file *fp)
{
	...
	if (0 == atomic_read(&driver_data->open_count))
	{
		...
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::hIntrGpioPin is NULL\n", __func__);
			up(&sem_open);
			return -1;
		}

		if (!NvOdmGpioInterruptRegister(driver_data->hGpio, &driver_data->hIntrGpioIntr, driver_data->hIntrGpioPin, NvOdmGpioPinMode_InputInterruptRisingEdge, gsensor_irq_handler, (void*)driver_data, 0))
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::NvOdmGpioInterruptRegister fail!!\n", __func__);
			up(&sem_open);
			return -1;
		}
...
static long misc_gsensor_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		case GSENSOR_IOC_READ:
			...
			NvOdmOsMemset(&local_gsensor_ioctl, 0, sizeof(struct gsensor_ioctl_t));
		...
		case GSENSOR_IOC_READ_DIRECT:
			...
			NvOdmOsMemset(&local_gsensor_ioctl, 0, sizeof(struct gsensor_ioctl_t));
			NvOdmOsMemset(&local_gsensor_ioctl_accel, 0, sizeof(struct gsensor_ioctl_accel_t));
...
static struct platform_driver i2c_gsensor_driver = {
	.driver = {
		.owner = THIS_MODULE,
...
static int i2c_gsensor_probe(struct platform_device *pdev)
{
	...
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] %s+\n", __func__);
		...
		pConn = NvOdmPeripheralGetGuid(GSENSOR_EVT1B_GUID);
	if (!pConn)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s:: pConn == NULL\n", __func__);
		return -1;
	}
	gsensor_driver_data.pConnectivity = pConn;
	...
	for (i = 0; i < pConn->NumAddress; i++)
	{
		switch (pConn->AddressList[i].Interface)
		{
			case NvOdmIoModule_I2c:
				gsensor_driver_data.I2CAddr = (pConn->AddressList[i].Address << 1);
				gsensor_driver_data.I2CPort = pConn->AddressList[i].Instance;
				found |= 1;
				break;

			case NvOdmIoModule_Gpio:
				gsensor_driver_data.IntrGpioPort = pConn->AddressList[i].Instance;
				gsensor_driver_data.IntrGpioPin = pConn->AddressList[i].Address;
				found |= 2;
				break;
	...
	if (found != 3)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s:: found = %d err\n", __func__, found);
		return -1;
	}

	gsensor_driver_data.hGpio = NvOdmGpioOpen();
	if (!gsensor_driver_data.hGpio)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s:: NvOdmGpioOpen fail err\n", __func__);
		return -1;
	}

	gsensor_driver_data.hIntrGpioPin = NvOdmGpioAcquirePinHandle(gsensor_driver_data.hGpio, gsensor_driver_data.IntrGpioPort, gsensor_driver_data.IntrGpioPin);
	if (!gsensor_driver_data.hIntrGpioPin)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s:: NvOdmGpioAcquirePinHandle err\n", __func__);
		ret = -1;
		goto i2c_gsensor_probe_NvOdmGpioAcquirePinHandle_err;
	}

	NvOdmGpioConfig(gsensor_driver_data.hGpio, gsensor_driver_data.hIntrGpioPin, NvOdmGpioPinMode_InputData);

	gsensor_driver_data.hI2C = NvOdmI2cOpen(NvOdmIoModule_I2c, gsensor_driver_data.I2CPort);
	if (!gsensor_driver_data.hI2C)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s:: NvOdmI2cOpen err\n", __func__);
		ret = -1;
		goto i2c_gsensor_probe_NvOdmI2cOpen_err;
	}

	NvOdmOsMemset(&gsensor_ioctl_setting, 0, sizeof(struct gsensor_ioctl_setting_t));

	ret = gsensor_default_config(&gsensor_driver_data);
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::gsensor_default_config fail-ret:%d\n", __func__, ret);
		goto i2c_gsensor_probe_gsensor_default_config_err;
	}
	...
	if (ret)
	{
		...
		goto i2c_gsensor_probe_misc_register_err;
	}
	...
	#if 1
	if (!gsensor_driver_data_p->hIntrGpioPin)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::hIntrGpioPin is NULL\n", __func__);
		ret = -1;
		goto i2c_gsensor_probe_hIntrGpioPin_err;
	}

	if (gsensor_driver_data_p->hGpio && gsensor_driver_data_p->hIntrGpioPin)
	{
		if (!NvOdmGpioInterruptRegister(gsensor_driver_data_p->hGpio, &gsensor_driver_data_p->hIntrGpioIntr, gsensor_driver_data_p->hIntrGpioPin, NvOdmGpioPinMode_InputInterruptRisingEdge, gsensor_irq_handler, (void*)gsensor_driver_data_p, 0))
		{
			GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::NvOdmGpioInterruptRegister fail!!\n", __func__);
			ret = -1;
			goto i2c_gsensor_probe_NvOdmGpioInterruptRegister_err;
		}
	}

	tx_data = BMA150_RESET_INT_MASK;
	ret = i2c_gsensor_write(gsensor_driver_data_p->hI2C, BMA150_REG_RESET_INT, &tx_data, sizeof(tx_data));
	if (ret < 0)
	{
		GSENSOR_LOGE(KERN_ERR "[GSENSOR] %s::i2c_gsensor_write fail!!-ret:%d\n", __func__, ret);
		goto i2c_gsensor_probe_write_err;
	}
	#endif
	

	platform_set_drvdata(pdev, &gsensor_driver_data);

	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] %s-ret:%d\n", __func__, ret);
	return ret;

i2c_gsensor_probe_write_err:
	if (gsensor_driver_data_p->hGpio && gsensor_driver_data_p->hIntrGpioPin && gsensor_driver_data_p->hIntrGpioIntr)
	{
		NvOdmGpioInterruptUnregister(gsensor_driver_data_p->hGpio, gsensor_driver_data_p->hIntrGpioPin, gsensor_driver_data_p->hIntrGpioIntr);
		gsensor_driver_data_p->hIntrGpioIntr = NULL;
	}
i2c_gsensor_probe_NvOdmGpioInterruptRegister_err:
i2c_gsensor_probe_hIntrGpioPin_err:
	misc_deregister(&misc_gsensor_device);
i2c_gsensor_probe_misc_register_err:
i2c_gsensor_probe_gsensor_default_config_err:
	NvOdmI2cClose(gsensor_driver_data.hI2C);
i2c_gsensor_probe_NvOdmI2cOpen_err:
	NvOdmGpioConfig(gsensor_driver_data.hGpio, gsensor_driver_data.hIntrGpioPin, NvOdmGpioPinMode_Tristate);
	NvOdmGpioReleasePinHandle(gsensor_driver_data.hGpio, gsensor_driver_data.hIntrGpioPin);
i2c_gsensor_probe_NvOdmGpioAcquirePinHandle_err:
	NvOdmGpioClose(gsensor_driver_data.hGpio);
...
{
	int ret = 0;
	struct gsensor_driver_data_t *driver_data = platform_get_drvdata(pdev);
...
static int __init gsensor_init(void)
{
	...
	if (system_rev != HWID_UNKNOWN && system_rev >= EVT2)
	{
		NvOdmOsMemset(&gsensor_driver_data, 0, sizeof(struct gsensor_driver_data_t));
		ret = platform_driver_register(&i2c_gsensor_driver);
	}
...
static void __exit gsensor_exit(void)
{
	...
	if (system_rev != HWID_UNKNOWN && system_rev >= EVT2)
	{
		platform_driver_unregister(&i2c_gsensor_driver);
	}
	GSENSOR_LOGD(KERN_DEBUG "[GSENSOR] %s-\n", __func__);
}
