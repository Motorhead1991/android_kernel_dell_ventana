...
static NvRmGpioPinHandle s_hDatacardGpioPin[2]; 
...
static void __init tegra_setup_debug_uart(void)
{
	...
	if (!console_set_none) {
		...
		if (c) clk_put(c);
		tegra_pinmux_config_tristate_table(pinmux, nr_pins, TEGRA_TRI_TRISTATE);
	}
}
...
static void __init tegra_setup_hcd(void)
{
	...
	for (i=0; i<ARRAY_SIZE(tegra_hcd_platform); i++) {
		...
		if (p->IdPinDetectionType == NvOdmUsbIdPinType_Gpio) {
			...
			if (i == 2) plat->wakeup_nr = TEGRA_GPIO_PJ7;
#endif
...
#elif defined(CONFIG_BCOM4329_RFKILL)
static struct bcom4329_platform_data bcom4329_platform;
static struct platform_device bcom4329_device = {
	.name = "bcom4329-rfkill",
	.dev = {
		.platform_data = &bcom4329_platform,
	},
};
static noinline void __init tegra_setup_rfkill(void)
{
    ...
    bcom4329_platform.gpio_reset =
        bcom4329_platform.gpio_enable =
        bcom4329_platform.gpio_wakepin = 0;
	...
	for (i=0; i<con->NumAddress; i++) {
		if (con->AddressList[i].Interface == NvOdmIoModule_Gpio) {
            ...
            if (j == 1)
            {
			bcom4329_platform.gpio_reset = nr_gpio;
            rc = gpio_request(bcom4329_platform.gpio_reset, "bcom4329_reset");
            if (rc < 0) {
                printk("bcom4329_reset gpio_request failed\n");
            }
			j++;
			printk("gpio_reset %d\n", nr_gpio);
            }
            ...
            {
			bcom4329_platform.gpio_enable = nr_gpio;
            rc = gpio_request(bcom4329_platform.gpio_enable, "bcom4329_enable");
            if (rc < 0) {
                printk("bcom4329_enable gpio_request failed\n");
            }
			j++;
            ...
            {
			bcom4329_platform.gpio_wakepin = nr_gpio;
            rc = gpio_request(bcom4329_platform.gpio_wakepin, "bcom4329_wakepin");
            if (rc < 0) {
                printk("bcom4329_wakepin gpio_request failed\n");
            }
			printk("gpio_wakepin %d\n", nr_gpio);
            }
            else
            {
			printk("%s gpio unknown...(%d)\n", __func__, nr_gpio);
            }
		}
	}
	if (platform_device_register(&bcom4329_device))
		pr_err("%s: registration failed\n", __func__);
}
int bcom4329_get_wakepin(void)
{
    return bcom4329_platform.gpio_wakepin;
}
#else
...
static struct platform_device atmel_mXT224_touch_device =
{
    .name = "atmel_mXT224_touchscreen",
    .id   = -1,
};
#endif

#ifdef CONFIG_KEYPAD_TEGRA
...
static struct platform_device ATA2538_capkey_device =
{
    .name = "ATA2538_capkey",
    .id   = -1,
};
...
static struct platform_device gsensor_bma150 =
{
	.name = "gsensor_bma150_i2c",
	.id   = -1,
};
#endif
...
static struct platform_device ecompass_ak8973s =
{
	.name = "ecompass_ak8973s_i2c",
	.id   = -1,
};
#endif


#ifdef CONFIG_GPIOCTL
static struct platform_device gpioctl =
{
	.name = "gpioctl",
	.id   = -1,
};
...
static struct platform_device camera_daemon_device =
{
  .name = "camera_daemon",
  .id   = -1,
};
#if defined(CONFIG_LUNA_BATTERY) || defined(CONFIG_LUNA_BATTERY_EVT1B)
...
static struct resource tegra_watchdog_resources[] = {
	[0] = {
		.start = TEGRA_CLK_RESET_BASE,
		.end = TEGRA_CLK_RESET_BASE + TEGRA_CLK_RESET_SIZE - 1,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
...
static struct platform_device tegra_watchdog_device =
{
  .name = "tegra_wdt",
  .id   = -1,
  .num_resources	= ARRAY_SIZE(tegra_watchdog_resources),
  .resource	= tegra_watchdog_resources,
...
static struct platform_device luna_headsetdet_device = {
	.name = "headset",
	.id		= -1,
...
static struct platform_device *nvodm_devices[] __initdata = {
  ...
  &luna_led_device,
	...
	&usim_plug_device,
#endif
  &luna_headsetdet_device,
#ifdef CONFIG_TEGRA_WATCHDOG
	&tegra_watchdog_device,
#endif
};
...
static void __init tegra_setup_ftm(void)
{
    (void) platform_device_register(&ftm_device);
}
...
#define W_DISABLE_PIN_IND 0 
#define PRESET_PIN_IND    1  
#define W_DISABLE_PIN_NUM 1
#define PRESET_PIN_NUM    3
void luna_datacard_power_up_sequence(int up)
{
	NvRmGpioPinState state;

	if ((s_hDatacardGpioPin[W_DISABLE_PIN_IND]==0) || (s_hDatacardGpioPin[PRESET_PIN_IND]==0))
	{
		printk(KERN_INFO "%s: pin handle not available\n", __func__);
		return;
	}
	...
	if (up)
	{
		...
		msleep(25);
		state = NvRmGpioPinState_High;
	}else {
		state = NvRmGpioPinState_Low;
	}
	NvRmGpioWritePins(s_hGpioGlobal, &s_hDatacardGpioPin[W_DISABLE_PIN_IND], &state, 1);
	return;
}
EXPORT_SYMBOL(luna_datacard_power_up_sequence);

static void __init luna_datacard_power_on(void)
{
	...
	pConnectivity =
		NvOdmPeripheralGetGuid(NV_ODM_GUID('d','a','t','a','c','a','r','d'));
	if (!pConnectivity)
		return;
	...
	for (i = 0; i < pConnectivity->NumAddress; i++)
	{
		...
		NvRmPmuGetCapabilities(s_hRmGlobal,
			pConnectivity->AddressList[i].Address, &rail);
...
static void __init luna_setup_datacard(void)
{
	const NvOdmPeripheralConnectivity *pConnectivity = NULL;
	int i;
	...
	s_hDatacardGpioPin[W_DISABLE_PIN_IND] = s_hDatacardGpioPin[PRESET_PIN_IND] = 0;
	...
	for (i = 0; i < pConnectivity->NumAddress; i++)
	{
		switch (pConnectivity->AddressList[i].Interface)
		{
			case NvOdmIoModule_Gpio:
				if( PRESET_PIN_NUM == pConnectivity->AddressList[i].Address )
				{
					port = pConnectivity->AddressList[i].Instance;
					pin = pConnectivity->AddressList[i].Address;
					NvRmGpioAcquirePinHandle(s_hGpioGlobal, port, pin, &s_hDatacardGpioPin[PRESET_PIN_IND]);
				} else if ( W_DISABLE_PIN_NUM == pConnectivity->AddressList[i].Address ) {
					port = pConnectivity->AddressList[i].Instance;
					pin = pConnectivity->AddressList[i].Address;
					NvRmGpioAcquirePinHandle(s_hGpioGlobal, port, pin, &s_hDatacardGpioPin[0]);
				}
				break;
			default:
				break;
	...
	NvRmGpioConfigPins(s_hGpioGlobal, &s_hDatacardGpioPin[PRESET_PIN_IND], 1, NvRmGpioPinMode_Output);
	state = NvRmGpioPinState_High;
	NvRmGpioWritePins(s_hGpioGlobal, &s_hDatacardGpioPin[PRESET_PIN_IND], &state, 1);

	
	NvRmGpioConfigPins(s_hGpioGlobal, &s_hDatacardGpioPin[W_DISABLE_PIN_IND], 1, NvRmGpioPinMode_Output);
	state = NvRmGpioPinState_Low;
	NvRmGpioWritePins(s_hGpioGlobal, &s_hDatacardGpioPin[W_DISABLE_PIN_IND], &state, 1);

	
	luna_datacard_power_on();
...
void __init tegra_setup_nvodm(bool standard_i2c, bool standard_spi)
{
	...
	pr_err("[tegra_is_ap20_a03p: %d] \n", tegra_is_ap20_a03p());
        ...
        gpio_request(TEGRA_GPIO_PZ5, "set PZ5");
        gpio_direction_output(TEGRA_GPIO_PZ5, 1);
