...
#include <mach/luna_battery_evt1b.h>
...
static struct timespec time_interval = { 1*60*60, 0 }; 
...
static void pmlog_flush_to_file(void)
{
	...
	size = FORMAT_COLUMN_RN(buffer, PAGE_SIZE, "Wakeup counter - total time - longest time:");
	for (i=0; i<32; i+=4) {
		size += FORMAT_COLUMN_R(buffer+size, PAGE_SIZE-size, "%lu: ", i);
		size += FORMAT_COLUMN_SR(buffer+size, PAGE_SIZE-size, 21, "%5lu:%5lu:%5lu\n",
                wake_count[i], wake_time_arr[i][0], wake_time_arr[i][1]);
		size += FORMAT_COLUMN_R(buffer+size, PAGE_SIZE-size, "%lu: ", i+1);
		size += FORMAT_COLUMN_SR(buffer+size, PAGE_SIZE-size, 21, "%5lu:%5lu:%5lu\n",
                wake_count[i+1], wake_time_arr[i+1][0], wake_time_arr[i+1][1]);
		size += FORMAT_COLUMN_R(buffer+size, PAGE_SIZE-size, "%lu: ", i+2);
		size += FORMAT_COLUMN_SR(buffer+size, PAGE_SIZE-size, 21, "%5lu:%5lu:%5lu\n",
                wake_count[i+2], wake_time_arr[i+2][0], wake_time_arr[i+2][1]);
		size += FORMAT_COLUMN_R(buffer+size, PAGE_SIZE-size, "%lu: ", i+3);
		size += FORMAT_COLUMN_SR(buffer+size, PAGE_SIZE-size, 21, "%5lu:%5lu:%5lu\n",
...
static int __devinit pmlogd_init(void)
{
	...
	pmlog_wqueue = __create_workqueue("pmlogd", 1, 0, 1);
	if (!pmlog_wqueue) {
